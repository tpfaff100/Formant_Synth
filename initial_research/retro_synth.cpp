//clang++ -std=c++20 -O3 retro_synth.cpp -o retro_synth
#include <iostream>
#include <cmath>
#include <fstream>
#include <vector>
...
std::map<char, PhoneticRule> PHONEMES = {
    ...
    // Hard Plosives (with distinct noise transient bursts)
...
// Map selection IDs directly to their optimized phonetic sequence strings
std::map<int, std::pair<std::string, std::string>> MENU = {
    {1,  {"Pet 2001", "pet tu taused wan"}},
    {2,  {"t-r-s-80", "ti ar es eiti"}},
    {3,  {"apple two", "apl tu"}},
    {4,  {"NEC PC8201a", "en i si pi si eit tu zi ro wan ei"}},
    ...
    {11, {"HP 9825", "eits pi nin eit tu fiv"}},
    {12, {"HP 9000-712", "eits pi nin taused sevn wan tu"}},
    {13, {"HP2000", "eits pi tu taused"}},
    {14, {"HP3000", "eits pi tri taused"}}
};
...
void print_menu() {
    std::cout << "=== Vintage Hardware Speech Engine ===\n";
    for (const auto& [id, target] : MENU) {
        std::cout << "  [" << id << "] " << target.first << "\n";
    }
    std::cout << "Usage: ./retro_synth <number>\n";
}
...
int main(int argc, char* argv[]) {
    ...
    if (!MENU.contains(choice)) {
        std::cout << "[-] Invalid hardware token selection.\n";
        print_menu();
        return 1;
    }

    auto [label, phonetic_sequence] = MENU[choice];
    std::cout << "[*] Synthesizing Voice Frame for: \"" << label << "\"\n";

    std::string output_filename = "retro_output.wav";
    std::vector<short> pcm_data;
    double p0 = 0.0, p1 = 0.0, p2 = 0.0, p3 = 0.0;
    unsigned int noise_seed = 8675309;

    for (char c : phonetic_sequence) {
        if (c == ' ') {
            int space_samples = static_cast<int>(0.08 * SAMPLE_RATE);
    ...
    std::cout << "[+] Compiling execution complete.\n";
