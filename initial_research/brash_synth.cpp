//clang++ -std=c++20 -O3 brash_synth.cpp -o brash_synth
#include <iostream>
#include <cmath>
#include <fstream>
#include <vector>
...
const double F0_BASE = 135.0; // Higher baseline pitch to amplify the tense cadence
...
// Phoneme mapping with elevated formants and constricted bandwidths for a piercing tone
std::map<std::string, PhoneticRule> PHONEMES = {
    // VOWELS - Narrowed bandwidths (35-60Hz) compress energy into sharp, biting peaks
    ...
    // CONSONANTS
    {"P",  {.duration = 0.09, .start = {240,  800, 1350, 50, 60, 85},  .end = {550, 1200, 2450, 40, 45, 60}, .is_fricative = false, .is_plosive = true,  .burst_duration = 0.018}},
    {"T",  {.duration = 0.09, .start = {450, 2200, 3200, 60, 75, 110}, .end = {550, 1500, 2550, 40, 45, 60}, .is_fricative = false, .is_plosive = true,  .burst_duration = 0.022}},
    {"K",  {.duration = 0.11, .start = {400, 1950, 2850, 50, 65, 100}, .end = {600, 1300, 2450, 40, 45, 60}, .is_fricative = false, .is_plosive = true,  .burst_duration = 0.026}},
    ...
    {"S",  {.duration = 0.16, .start = {4800, 5800, 7200, 100, 150, 250}, .end = {4400, 5400, 6800, 100, 150, 250}, .is_fricative = true,  .is_plosive = false, .burst_duration = 0.0}},
    {"SH", {.duration = 0.18, .start = {2700, 3700, 4700, 90, 130, 200},  .end = {2400, 3400, 4400, 90, 130, 200},  .is_fricative = true,  .is_plosive = false, .burst_duration = 0.0}},
    {"Z",  {.duration = 0.14, .start = {3800, 4800, 5800, 90, 130, 200},  .end = {3300, 4300, 5300, 90, 130, 200},  .is_fricative = true,  .is_plosive = false, .burst_duration = 0.0}},
    {"F",  {.duration = 0.14, .start = {2200, 3200, 4200, 150, 200, 300}, .end = {2000, 3000, 4000, 150, 200, 300}, .is_fricative = true,  .is_plosive = false, .burst_duration = 0.0}},
    {"V",  {.duration = 0.12, .start = {350,  1300, 2300, 50, 85, 140},  .end = {280,  1200, 2200, 50, 85, 140},  .is_fricative = true,  .is_plosive = false, .burst_duration = 0.0}},
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./brash_synth <number (1-14)>\n";
        return 1;
    }
    ...
    std::cout << "[*] Driving brash, high-saturation filters for: \"" << label << "\"\n";

    std::string output_filename = "brash_output.wav";
    ...
    for (const std::string& tok : phoneme_vector) {
        if (tok == " ") {
            int space_samples = static_cast<int>(0.04 * SAMPLE_RATE); // Snappier word gaps
        ...
        for (int s = 0; s < total_samples; ++s) {
            ...
            if (s < burst_samples) {
                source_signal = noise * (1.0 - (static_cast<double>(s) / burst_samples)) * 1.5;
            } else if (ph.is_fricative) {
                source_signal = noise * 0.85; // Hotter sibilance mix
            } else {
                // Harmonic Saturation: Overdriving the pulse train for abrasive odd harmonics
            ...
            // Boosted output gains for aggressive delivery
            double gain = ph.is_fricative ? 0.12 : 0.45;
            double sample_val = filtered * gain;

            // Tight transient windowing
    ...
    std::cout << "[+] Brash compile absolute. Rendering output...\n";
