...
clang++ -std=c++20 -O3 ringmod_tts.cpp -o ringmod_tts

To test the scaling effect, run a short sentence followed by a long sentence. You will notice that the metallic modulation stretches out to match the length of the string:
Bash

# Ring Modulation decays quickly over a short sentence
./ringmod_tts --filter-sine "six quick jazz files"

# Ring Modulation decays slowly and proportionally over a longer sentence
./ringmod_tts --filter-glitch "i just want to make a quick update because project glasswing execution needs six more jazz files"
*/
#include <iostream>
#include <cmath>
#include <fstream>
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./ringmod_tts [--filter-sine | --filter-singing | --filter-glitch] \"<phrase>\"\n";
        return 1;
    }
    ...
    if (raw_string.empty()) {
        std::cout << "[-] Missing string sequence input text.\n";
        return 1;
    }
    ...
    std::string word;
    double total_stream_duration = 0.0;

    // First pass structural layout: map phrases and accumulate text metrics
    while (ss >> word) {
        ...
        for (const auto& ph : targeted_phonemes) {
            ...
            if (PHONEMES.contains(ph)) {
                total_stream_duration += PHONEMES[ph].duration;
            }
        }
        master_stream.push_back({" ", target_to_sine, is_singing}); 
        total_stream_duration += 0.06; // add inter-word blank boundaries
    }

    std::string filename = "ringmod_sweep_output.wav";
    ...
    // Define time constant proportional to total length of the sequence
    // This scales the exponential decay curve gracefully
    double time_constant_tau = total_stream_duration / 2.2; 
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            // State Variable Filter pass (0.5 Hz Modulation Engine)
            ...
            morphFilter.process(raw_sample, targetCutoff, 2.2, lpSample, hpSample);
            double filtered_sample = (lpSample * (1.0 - filterLfo)) + (hpSample * filterLfo);

            // --- Text Duration Adaptive Ring Modulation Matrix ---
            phase_ring_mod += (2.0 * PI * 440.0) / SAMPLE_RATE; // 440Hz metallic carrier signal
            if (phase_ring_mod > 2.0 * PI) phase_ring_mod -= 2.0 * PI;

            // Exponentially decay ring modulation mix based on the total audio footprint length
            double ringModEnvelope = std::exp(-currentTime / time_constant_tau);
            double modulated_signal = filtered_sample * std::sin(phase_ring_mod);

            // Crossfade processed signal from heavy metallic ring mod to clean filtered speech
            double final_sample = (modulated_signal * ringModEnvelope) + (filtered_sample * (1.0 - ringModEnvelope));

            if (s < 80) final_sample *= ((double)s / 80.0);
            if (s > len - 150) final_sample *= ((double)(len - s) / 150.0);

            final_sample = std::clamp(final_sample, -1.0, 1.0);
            pcm.push_back(static_cast<short>(final_sample * 32767));
            global_sample_index++;
        }
    }
    ...
    std::cout << "[+] Rendering complete. Sequence length calculated at: " << total_stream_duration << " seconds.\n";
