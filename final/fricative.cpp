//clang++ -std=c++20 -O3 fricative.cpp -o fricative 
...
std::map<std::string, PhoneticRule> PHONEMES = {
    ...
    // New Phonetic Entry: "J" Affricate sound modeling
    // Combines an instantaneous explosive start (burst) with a resonant mid-frequency noise body release
...
std::map<std::string, std::vector<std::string>> LEXICON = {
    ...
    // Added dictionary terms tracking vocabulary containing 'J'
    {"jazz", {"J", "AE", "Z"}}, {"just", {"J", "AH", "S", "T"}}, {"project", {"P", "R", "AA", "J", "EH", "K", "T"}}
};
...
std::vector<std::string> rule_based_g2p(const std::string& word) {
    ...
    while (i < word.length()) {
        ...
        // Direct tracking pipeline mapping character 'j' to our new phonetic affricate entries
        if (c == 'j') tokens.push_back("J");
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./affricate_tts [--hybrid | --sine | --singing | --alternate | --alternate-singing | --glitch-singing | --alternate-glitch] \"<phrase>\"\n";
        return 1;
    }
    ...
    if (raw_string.empty()) {
        std::cout << "[-] Missing string data target stream.\n";
        return 1;
    }
    ...
    std::string filename = "affricate_output.wav";
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.use_sine_engine) {
                ...
                if (ph.is_plosive && s < burst) {
                    // Affricate explosive burst start handling inside the additive sine engine
                    sample_val = std::sin(phase_f1 * 2.8) * (1.0 - ((double)s / burst)) * 0.55;
                    ...
                    sample_val = std::sin(phase_fric_a) * std::sin(phase_fric_b * 1.5) * 0.22;
                } else {
                ...
                // Advanced hybrid stop/fricative sequencing
                if (ph.is_plosive && ph.is_fricative) {
                    // Unique "J" logic path: First 25ms mimics a plosive burst, 
                    // then cleanly crossfades directly into the high-friction white noise body
                    if (s < burst) {
                        src = noise_lpf_state * (1.2 - ((double)s / burst)) * 1.8;
                    } else {
                        src = noise_lpf_state * 0.65;
                    }
                } else if (s < burst) {
                ...
                sample_val = R3.process(R2.process(R1.process(src))) * (ph.is_fricative ? 0.16 : 0.45);
            }
    ...
    std::cout << "[+] Rendering updated affricate stream complete.\n";
