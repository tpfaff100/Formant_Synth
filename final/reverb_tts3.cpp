// Compiles clean with c++20 features
// clang++ -std=c++20 -O3 reverb_tts3.cpp -o reverb_tts3
//
// Short Modulation Period (setting 0 = 1200Hz hyper-metallic ring)
// ./reverb_tts --filter-singing --ringmod 0 --reverb 10 "project executive"
//
// Long Modulation Period (setting 20 = 40Hz deep slow vibrato/tremolo throb)
// ./reverb_tts --filter-singing --ringmod 20 --reverb 10 "project executive"
//
// Balanced mid-range space drone (setting 10 = 620Hz buzz)
// ./reverb_tts --filter-glitch --ringmod 10 --reverb 15 "project executive"
//
///////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./reverb_tts [--filter-sine | --filter-singing | --filter-glitch] [--filter-ringmod 0-20] [--reverb 0-20] \"<phrase>\"\n"; return 1;
    }
    EngineMode selected_mode = MODE_FILTER_SINE; std::string raw_string = ""; 
    int reverbValue = 0;
    int ringmodValue = -1;
    ...
    std::string filename = "reverb_output.wav"; std::vector<short> pcm;
    ...
    for (const auto& scheduled : master_stream) {
        if (scheduled.token == " ") {
            ...
            for (int s = 0; s < gap; ++s) { pcm.push_back(acousticReverb.process(0.0) * 32767); global_sample_index++; }
            continue;
        }
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.is_singing_word) {
                ...
                // Restored Pitch Glitch Tangent Function
            ...
            pcm.push_back(static_cast<short>(processed_sample * 32767)); global_sample_index++;
        }
    }
    ...
    std::cout << "[+] Rendering complete. Reverb size: " << reverbValue << " (" << reverbDuration << "s). ";
    if (enable_ring_mod) std::cout << "Ringmod setting: " << ringmodValue << " (" << rmCarrierHz << " Hz)\n";
    else std::cout << "Ringmod: OFF\n";
