// Minimal 0.01 second room size (near bone-dry)
// ./reverb_tts --filter-singing --ringmod --reverb 0 "project executive"
//
// Huge space simulation at setting 12 (~3.0 seconds of decay time)
// ./reverb_tts --filter-glitch --ringmod --reverb 12 "project executive"
//
// Maximum setting 20 (Full 5.0 second vast echo chamber bloom)
// ./reverb_tts --filter-glitch --reverb 20 "project executive"
//
// Compile like so:
// clang++ -std=std=c++20 -O3 reverb_tts2.cpp -o reverb_tts
//
////////////////////////////////////////////////////////////////////
...
public:
    ...
    void initialize(double durationSec) {
        ...
        // Scale wet mix smoothly from subtle room ambience up to prominent cathedral wash
        activeWetMix = 0.15 + (std::min(durationSec, 3.5) * 0.12);
        
        // Expanded prime prime-boundary sample intervals to accommodate up to 5.0 seconds
        int cDelays[4] = {1103, 1249, 1399, 1559}; 
        int aDelays[2] = {211, 89};
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./reverb_tts [--filter-sine | --filter-singing | --filter-glitch] [--ringmod] [--reverb 0-20] \"<phrase>\"\n"; return 1;
    }
    EngineMode selected_mode = MODE_FILTER_SINE; std::string raw_string = ""; int reverbValue = 0;
    bool enable_ring_mod = false;
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--ringmod") enable_ring_mod = true;
    ...
    // Clamp to new 0-20 parameter spectrum limits
    reverbValue = std::clamp(reverbValue, 0, 20);
    
    // Scale mapping calculation: 0 -> 0.01s, 20 -> 5.0s
    ...
    // Flush the long 5-second tails out beyond the end of the text sequence
