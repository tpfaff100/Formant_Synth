//clang++ -std=c++20 -O3 fricative2.cpp -o fricative2 
#include <iostream>
#include <cmath>
#include <fstream>
#include <vector>
...
std::map<std::string, std::vector<std::string>> LEXICON = {
    ...
    {"project", {"P", "R", "AA", "J", "EH", "K", "T"}},
    // Added dictionary expansions to support testing composite character structures natively
...
std::vector<std::string> rule_based_g2p(const std::string& word) {
    ...
    while (i < word.length()) {
        ...
        // Composite Cluster Resolution Pipeline Updates
        if (c == 'q') {
            tokens.push_back("K"); tokens.push_back("W"); // 'q' expands to structural matrix /kw/
        } else if (c == 'x') {
            tokens.push_back("K"); tokens.push_back("S"); // 'x' expands to structural matrix /ks/
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./composite_tts [--hybrid | --sine | --singing | --alternate | --alternate-singing | --glitch-singing | --alternate-glitch] \"<phrase>\"\n";
        return 1;
    }
    ...
    if (raw_string.empty()) {
        std::cout << "[-] Missing string core input stream token.\n";
        return 1;
    }
    ...
    std::string filename = "composite_output.wav";
    ...
    std::cout << "[+] Rendering complete with expanded composite cluster definitions.\n";
