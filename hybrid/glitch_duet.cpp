//clang++ -std=c++20 -O3 glitch_duet.cpp -o glitch_duet
//./glitch_duet --alternate-glitch --start-hybrid "ooga booga ooga booga"
//./glitch_duet --alternate-glitch --start-sine "ooga booga ooga booga"
...
enum EngineMode { MODE_HYBRID, MODE_SINE, MODE_ALTERNATE, MODE_SINGING, MODE_ALTERNATE_SINGING, MODE_GLITCH_SINGING, MODE_ALTERNATE_GLITCH };
enum StartEngine { START_WITH_HYBRID, START_WITH_SINE };
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./glitch_duet [--hybrid | --sine | --singing | --alternate | --alternate-singing | --glitch-singing | --alternate-glitch] [--start-hybrid | --start-sine] \"<phrase>\"\n";
        return 1;
    }

    EngineMode selected_mode = MODE_ALTERNATE;
    StartEngine starting_engine = START_WITH_HYBRID;
    ...
    for(size_t i = 0; i < args.size(); ++i) {
        if (args[i] == "--hybrid") selected_mode = MODE_HYBRID;
        else if (args[i] == "--sine") selected_mode = MODE_SINE;
        else if (args[i] == "--singing") selected_mode = MODE_SINGING;
        else if (args[i] == "--alternate") selected_mode = MODE_ALTERNATE;
        else if (args[i] == "--alternate-singing") selected_mode = MODE_ALTERNATE_SINGING;
        else if (args[i] == "--glitch-singing") selected_mode = MODE_GLITCH_SINGING;
        else if (args[i] == "--alternate-glitch") selected_mode = MODE_ALTERNATE_GLITCH;
        else if (args[i] == "--start-hybrid") starting_engine = START_WITH_HYBRID;
        else if (args[i] == "--start-sine") starting_engine = START_WITH_SINE;
        else {
            if (raw_string.empty()) raw_string = args[i];
        }
    }

    if (raw_string.empty()) {
        std::cout << "[-] Text context literal missing.\n";
        return 1;
    }
    ...
    std::cout << "[*] Parsing Complete. Pipeline Mode: ";
    if (selected_mode == MODE_ALTERNATE_GLITCH) std::cout << "Alternating Mechanical Sine & Tangent Glitch Voice\n";
    else std::cout << "Fallback Pipeline Target Engaged\n";
    ...
    while (ss >> word) {
        ...
        bool is_even = (word_count % 2 == 0);

        if (selected_mode == MODE_SINE) {
            target_to_sine = true;
        } else if (selected_mode == MODE_SINGING || selected_mode == MODE_GLITCH_SINGING) {
            is_singing = true;
        } else if (selected_mode == MODE_ALTERNATE) {
            target_to_sine = (starting_engine == START_WITH_HYBRID) ? is_even : !is_even;
        } else if (selected_mode == MODE_ALTERNATE_SINGING || selected_mode == MODE_ALTERNATE_GLITCH) {
            if (starting_engine == START_WITH_HYBRID) {
                // Hybrid/Glitch path gets the odd words, flat geometric sine gets the even words
                target_to_sine = is_even; is_singing = !is_even;
            } else {
                // Flat geometric sine gets the odd words, hybrid/glitch path gets the even words
                target_to_sine = !is_even; is_singing = is_even;
            }
        }
    ...
    std::string filename = "alternate_glitch.wav";
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.is_singing_word) {
                double t = (double)global_sample_index / SAMPLE_RATE;
                double base_melody = 165.0 + 45.0 * std::sin(2.0 * PI * 0.85 * t);
                double vibrato = 8.0 * std::sin(2.0 * PI * 6.2 * t);
                current_f0 = base_melody + vibrato;

                // Evaluate if tangent explosion conditions are active
                if (selected_mode == MODE_GLITCH_SINGING || selected_mode == MODE_ALTERNATE_GLITCH) {
                    double glitch_window = std::sin(2.0 * PI * 0.4 * t);
                    if (glitch_window > 0.3) {
                        double tan_mod = std::tan(2.0 * PI * 18.0 * t);
                        current_f0 += tan_mod * 85.0;
                    }
                }
            }

            // Boundary clamping matrix protections
            if (std::isnan(current_f0) || std::isinf(current_f0) || current_f0 < 10.0) {
                current_f0 = 40.0; 
            } else if (current_f0 > 4500.0) {
                current_f0 = 4500.0;
            }
            ...
            if (scheduled.use_sine_engine) {
                ...
                if (ph.is_fricative) {
                    ...
                    sample_val = std::sin(phase_fric_a) * std::sin(phase_fric_b * 1.77) * 0.25;
                } else if (s < burst) {
                    sample_val = std::sin(phase_f1 * 2.5) * (1.0 - ((double)s / burst)) * 0.4;
                } else {
                    sample_val = (std::sin(phase_f0) * 0.4) + (std::sin(phase_f1) * 0.35) + 
                ...
                double src = 0.0;
                if (s < burst) src = noise_lpf_state * (1.0 - ((double)s / burst)) * 1.5;
                else if (ph.is_fricative) src = noise_lpf_state * 0.70;
                else { double p = get_glottal_pulse(hybrid_p0) * 1.6; src = p - 0.15 * std::pow(p, 3); }

                sample_val = R3.process(R2.process(R1.process(src))) * (ph.is_fricative ? 0.14 : 0.45);
            }
    ...
    std::cout << "[+] Rendering vector complete. Initializing local execution loop.\n";
