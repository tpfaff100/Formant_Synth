//clang++ -std=c++20 -O3 flex_tts.cpp -o flex_tts
//usage:  ./flex_tts --start-hybrid "we think you want some people to look back"
//./flex_tts --start-sine "we think you want some people to look back"

/*To give you full control over the starting alignment when alternating, we can add a secondary flag configuration: --start-hybrid or --start-sine.

When running in alternation mode, this setting changes whether the odd-indexed words or the even-indexed words get routed to the sine synthesizer. If you don't provide a starting preference, it defaults to --start-hybrid (matching the previous behavior).

Here is the updated source file containing the runtime evaluation logic for the alternation initialization path.
Configurable Alignment Speech Engine (flexible_tts.cpp)
*/
...
enum EngineMode { MODE_HYBRID, MODE_SINE, MODE_ALTERNATE };
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./flexible_tts [--hybrid | --sine | --start-hybrid | --start-sine] \"<text phrase>\"\n";
        return 1;
    }
    ...
    // Comprehensive parsing matrices for positional flags
    if (argc >= 3) {
        std::string flag = argv[1];
        if (flag == "--hybrid") selected_mode = MODE_HYBRID;
        else if (flag == "--sine") selected_mode = MODE_SINE;
        else if (flag == "--start-hybrid") { selected_mode = MODE_ALTERNATE; starting_engine = START_WITH_HYBRID; }
        else if (flag == "--start-sine") { selected_mode = MODE_ALTERNATE; starting_engine = START_WITH_SINE; }
        raw_string = argv[2];
    } else {
        raw_string = argv[1];
    }
    ...
    std::cout << "[*] Routing Setup initialized. Configured state: ";
    if (selected_mode == MODE_HYBRID) std::cout << "Static Hybrid Subtractive\n";
    else if (selected_mode == MODE_SINE) std::cout << "Static Pure Additive Sine\n";
    else {
        std::cout << "Alternating Words (Starting Engine: " 
                  << (starting_engine == START_WITH_HYBRID ? "Hybrid Filter" : "Additive Sine") << ")\n";
    }
    ...
    while (ss >> word) {
        ...
        if (selected_mode == MODE_SINE) {
            ...
            if (starting_engine == START_WITH_HYBRID) {
                target_to_sine = is_even; // Even words map to sines
            } else {
                target_to_sine = !is_even; // Odd words map to sines
            }
        }
        ...
        for (const auto& ph : targeted_phonemes) {
            master_stream.push_back({ph, target_to_sine});
        }
        master_stream.push_back({" ", target_to_sine}); 
        word_count++;
    }

    std::string filename = "flexible_output.wav";
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.use_sine_engine) {
                ...
                hybrid_p0 += (2.0 * PI * F0_BASE) / SAMPLE_RATE;
