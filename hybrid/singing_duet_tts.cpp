//clang++ -std=c++20 -O3 singing_duet_tts.cpp -o duet_tts
//ex:  ./duet_tts --alternate-singing --start-sine "radio shack"
#include <iostream>
#include <cmath>
#include <fstream>
#include <vector>
...
enum EngineMode { MODE_HYBRID, MODE_SINE, MODE_ALTERNATE, MODE_SINGING, MODE_ALTERNATE_SINGING };
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./duet_tts [--hybrid | --sine | --singing | --alternate | --alternate-singing] [--start-hybrid | --start-sine] \"<phrase>\"\n";
        return 1;
    }
    ...
    // Comprehensive multi-flag parsing pipeline
    ...
    for(size_t i = 0; i < args.size(); ++i) {
        if (args[i] == "--hybrid") selected_mode = MODE_HYBRID;
            ...
            // Assume the final non-flag argument contains our sentence payload string
            if (raw_string.empty()) raw_string = args[i];
        }
    }

    if (raw_string.empty()) {
        std::cout << "[-] Missing payload string literal to compile. Exit.\n";
        return 1;
    }
    ...
    std::cout << "[*] Architectural Core Mapped. Execution Profile: ";
    if (selected_mode == MODE_HYBRID) std::cout << "Static Subtractive Filter Base\n";
    else if (selected_mode == MODE_SINE) std::cout << "Static Pure Additive Geometric Sines\n";
    else if (selected_mode == MODE_SINGING) std::cout << "Monophonic Singing Engine\n";
    else if (selected_mode == MODE_ALTERNATE) std::cout << "Standard Duet (Flat Sine vs Flat Hybrid)\n";
    else std::cout << "Dynamic Singing Alternation (Flat Sine vs Singing Hybrid) 🎵✨\n";
    ...
    while (ss >> word) {
        ...
        if (selected_mode == MODE_SINE) {
            target_to_sine = true;
        } else if (selected_mode == MODE_SINGING) {
            is_singing = true;
            ...
            // Split pipeline: One path maps directly to flat geometric sines, 
            // the alternating path maps to the sweeping vocal hybrid matrix.
            if (starting_engine == START_WITH_HYBRID) {
                // Odd words = singing, Even words = flat sine
                target_to_sine = is_even;
                is_singing = !is_even;
            } else {
                // Odd words = flat sine, Even words = singing
                target_to_sine = !is_even;
                is_singing = is_even;
            }
        }
    ...
    std::string filename = "duet_output.wav";
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            // Calculate pitch shift matrix targets dynamically
    ...
    std::cout << "[+] Processing complete. Executing system audio pipeline...\n";
