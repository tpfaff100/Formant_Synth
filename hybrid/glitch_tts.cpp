//clang++ -std=c++20 -O3 glitch_tts.cpp -o glitch_tts
//./glitch_tts --glitch-singing "i want some people to look back at the commodore sixty four executive"
...
enum EngineMode { MODE_HYBRID, MODE_SINE, MODE_ALTERNATE, MODE_SINGING, MODE_ALTERNATE_SINGING, MODE_GLITCH_SINGING };
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./glitch_tts [--hybrid | --sine | --singing | --alternate | --alternate-singing | --glitch-singing] \"<phrase>\"\n";
        return 1;
    }
    ...
    if (raw_string.empty()) {
        std::cout << "[-] Input string missing. Exit.\n";
        return 1;
    }
    ...
    std::cout << "[*] System configuration set. Routing style: ";
    if (selected_mode == MODE_GLITCH_SINGING) std::cout << "Tangent-Modulated Glitch-Singing Core 💥⚡\n";
    else std::cout << "Standard Modes\n";
    ...
    while (ss >> word) {
        ...
        } else if (selected_mode == MODE_ALTERNATE_SINGING) {
    ...
    std::string filename = "glitch_output.wav";
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.is_singing_word) {
                ...
                // Tangent Interruption Engine Activation
                if (selected_mode == MODE_GLITCH_SINGING) {
                    // Slow oscillation tracking the "destruction windows" (roughly every 2.5 seconds)
                    double glitch_window = std::sin(2.0 * PI * 0.4 * t);
                    
                    // If the macro window breaches threshold, feed a fast-moving time value to tan()
                    if (glitch_window > 0.3) {
                        double tan_mod = std::tan(2.0 * PI * 18.0 * t);
                        // Amplify and apply to fundamental frequency vector
                        current_f0 += tan_mod * 85.0;
                    }
                }
            }

            // Boundary safeguard: Keep pitch within safely audible, non-destructive ranges
    ...
    std::cout << "[+] Rendering complete. Sending to afplay pipeline.\n";
