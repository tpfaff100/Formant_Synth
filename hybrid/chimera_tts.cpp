//clang++ -std=c++20 -O3 chimera_tts.cpp -o chimera_tts
/*
 could you build a combination of thse two programs that takes the every odd numbered word and sends it to the hybrid software and every even word and sends it to the sine based software?

Here is the chimera engine. This version splits the input string into a word stream, tracks the index, and alternates the processing logic on a word-by-word basis.

    Odd-numbered words (1st, 3rd, 5th...) are routed through the Subtractive Hybrid Engine, utilizing the glottal pulse wave, white noise generator, and the triple resonant filter cascade.

    Even-numbered words (2nd, 4th, 6th...) are routed through the Additive Sine Engine, using purely stacked and modulated sinusoidal wave carriers.

Because the two synthesis methods output at vastly different relative amplitudes, a scaling factor has been integrated to keep the volume levels balanced as the voice morphs between styles.
Alternating Topology Speech Engine
*/
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./chimera_tts \"<phrase string>\"\n";
        return 1;
    }
    ...
    int word_count = 1; // 1-based indexing: Odd = Hybrid, Even = Sine

    std::cout << "[*] Front-End Routing Engine Mapping:\n";
    while (ss >> word) {
        std::string clean = clean_str(word);
        if (clean.empty()) continue;

        bool is_even_sine = (word_count % 2 == 0);
        std::cout << "  -> Word #" << word_count << " \"" << word << "\" scheduled for: " 
                  << (is_even_sine ? "[PURE SINE ENGINE]\n" : "[HYBRID RE-RESONATOR ENGINE]\n");

        std::vector<std::string> targeted_phonemes;
        if (LEXICON.contains(clean)) {
            targeted_phonemes = LEXICON[clean];
        } else {
            targeted_phonemes = rule_based_g2p(clean);
        }

        for (const auto& ph : targeted_phonemes) {
            master_stream.push_back({ph, is_even_sine});
        }
        master_stream.push_back({" ", is_even_sine}); // Gap tracking
        word_count++;
    }

    std::string filename = "chimera_output.wav";
    std::vector<short> pcm;

    // Phase storage for additive engines
    double phase_f0 = 0.0, phase_f1 = 0.0, phase_f2 = 0.0, phase_f3 = 0.0;
    double phase_fric_a = 0.0, phase_fric_b = 0.0;
    
    // Memory nodes for hybrid subtractive engine
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            if (scheduled.use_sine_engine) {
                // --- EVEN WORD ENGINE: ADDITIVE SINES ---
                ...
                // Gain balance normalization for sine topology matching
                sample_val *= 0.85;

            } else {
                // --- ODD WORD ENGINE: SUBTRACTIVE RESONATORS ---
            ...
            // Global crossfade windows to avoid node connection snaps
    ...
    std::cout << "[+] Chimera alternate pipeline complete. File written to: " << filename << "\n";
