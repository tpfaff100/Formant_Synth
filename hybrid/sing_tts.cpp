//clang++ -std=c++20 -O3 singing_tts.cpp -o sing_tts
#include <iostream>
#include <cmath>
#include <fstream>
#include <vector>
...
enum EngineMode { MODE_HYBRID, MODE_SINE, MODE_ALTERNATE, MODE_SINGING };
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./singing_tts [--hybrid | --sine | --start-hybrid | --start-sine | --singing] \"<text phrase>\"\n";
        return 1;
    }
    ...
    if (argc >= 3) {
        ...
        else if (flag == "--singing") selected_mode = MODE_SINGING;
    ...
    std::cout << "[*] Initializing engine. Mode: ";
    ...
    else if (selected_mode == MODE_SINGING) std::cout << "Whimsical Singing Hybrid Subtractive 🎵\n";
    else std::cout << "Alternating Engines\n";
    ...
    while (ss >> word) {
        ...
        } // For MODE_SINGING, we route to the hybrid path (target_to_sine = false)
    ...
    std::string filename = "singing_output.wav";
    ...
    // Time counter tracking overall sample progression for global melody modulation
    ...
    for (const auto& scheduled : master_stream) {
        ...
        for (int s = 0; s < len; ++s) {
            ...
            // Establish pitch target
            double current_f0 = F0_STATIC;

            if (selected_mode == MODE_SINGING) {
                double t = (double)global_sample_index / SAMPLE_RATE;
                // Base slow operatic melody wave (cycles pitch between roughly 120Hz and 210Hz)
                double base_melody = 165.0 + 45.0 * std::sin(2.0 * PI * 0.85 * t);
                // Fast retro hardware vibrato (6 Hz oscillation)
    ...
    std::cout << "[+] Audio processing sequence complete.\n";
