...
// STEREO MODIFICATION: Num channels changed to 2, byte rates updated accordingly
void write_wav_header(std::ofstream& file, int data_size) {
    ...
    short af = 1; 
    short nc = 2; // Stereo Channels
    int sr = SAMPLE_RATE; 
    int br = SAMPLE_RATE * 2 * 2; // 16-bit stereo byte rate
    short ba = 4; // 4 bytes per block frame (2 channels * 2 bytes)
    short bps = 16;
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./reverb_tts [--filter-sine | --filter-singing | --filter-glitch] [--ringmod | --filter-ringmod 0-20] [--reverb 0-20] [--panning | --filter-pan 0-20] \"<phrase>\"\n"; return 1;
    }
    ...
    int reverbValue = 0;
    int ringmodValue = -1;
    int panValue = -1; // -1 means disabled/centered mono split
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        else if ((arg == "--ringmod" || arg == "--filter-ringmod") && i + 1 < argc) { ringmodValue = std::atoi(argv[++i]); }
        else if (arg == "--reverb" && i + 1 < argc) { reverbValue = std::atoi(argv[++i]); }
        else if ((arg == "--panning" || arg == "--filter-pan") && i + 1 < argc) { panValue = std::atoi(argv[++i]); }
        else {
    ...
    reverbValue = std::clamp(reverbValue, 0, 20);
    double reverbDuration = 0.01 + ((double)reverbValue / 20.0) * (5.0 - 0.01);

    double rmCarrierHz = 0.0;
    bool enable_ring_mod = (ringmodValue >= 0);
    if (enable_ring_mod) {
        ringmodValue = std::clamp(ringmodValue, 0, 20);
        double norm = (double)ringmodValue / 20.0;
        rmCarrierHz = 1200.0 - (norm * (1200.0 - 40.0));
    }

    double panLfoHz = 0.0;
    bool enable_panning = (panValue >= 0);
    if (enable_panning) {
        panValue = std::clamp(panValue, 0, 20);
        panLfoHz = 0.1 + ((double)panValue / 20.0) * (6.0 - 0.1); // Sweeps between 0.1Hz and 6Hz
    }
    ...
    while (ss >> word) {
        ...
        bool target_to_sine = (selected_mode == MODE_FILTER_SINE);
        bool is_singing = (selected_mode == MODE_FILTER_SINGING || selected_mode == MODE_FILTER_GLITCH);
    ...
    std::string filename = "stereo_output1.wav"; 
    std::vector<short> pcm; // Interleaved left/right samples
    ...
    for (const auto& scheduled : master_stream) {
        if (scheduled.token == " ") {
            ...
            for (int s = 0; s < gap; ++s) { 
                ...
                short pcmVal = static_cast<short>(outSample * 32767);
                pcm.push_back(pcmVal); pcm.push_back(pcmVal); // Left & Right Identical inside gaps
                global_sample_index++; 
            }
            continue;
        }
        ...
        for (int s = 0; s < len; ++s) {
            ...
            // STEREO MODIFICATION: Constant-Power Panning calculation
            double leftGain = 0.7071;
            double rightGain = 0.7071;
            if (enable_panning) {
                // Generate a cyclic LFO panning coefficient sweeping from 0.0 to 1.0
                double panControl = 0.5 + 0.5 * std::sin(2.0 * PI * panLfoHz * currentTime);
                leftGain = std::cos(panControl * (PI / 2.0));
                rightGain = std::sin(panControl * (PI / 2.0));
            }

            short pcmLeft = static_cast<short>(processed_sample * leftGain * 32767);
            short pcmRight = static_cast<short>(processed_sample * rightGain * 32767);

            // Interleave stereo frame channels
    ...
    for (int t = 0; t < tailSamples; ++t) {
        ...
        pcm.push_back(static_cast<short>(tail * leftGain * 32767));
        pcm.push_back(static_cast<short>(tail * rightGain * 32767));
        global_sample_index++;
    }
    ...
    std::cout << "[+] Reverb size: " << reverbValue << " (" << reverbDuration << "s).\n";
    if (enable_ring_mod) std::cout << "[+] Ringmod setting: " << ringmodValue << " (" << rmCarrierHz << " Hz)\n";
    if (enable_panning) std::cout << "[+] Constant-Power Stereo Panning LFO Rate: " << panLfoHz << " Hz\n";
    else std::cout << "[+] Panning: Centered Mono Split\n";
