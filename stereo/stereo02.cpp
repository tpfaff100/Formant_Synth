...
enum EngineMode { MODE_FILTER_SINE, MODE_FILTER_SINGING, MODE_FILTER_GLITCH };
...
struct ScheduledPhoneme { std::string token; bool use_sine_engine; bool is_singing_word; };

std::vector<std::string> rule_based_g2p(const std::string& word) {
    std::vector<std::string> tokens; size_t i = 0;
    while (i < word.length()) {
        char c = word[i];
        if (i + 1 < word.length()) {
...
std::string clean_str(const std::string& input) {
    std::string out = ""; for (char c : input) if (std::isalnum(c)) out += std::tolower(c); return out;
}

class Resonator {
private: double a, b, c, y1, y2;
public:
    ...
    double process(double in) {
        double out = a * in + b * y1 + c * y2; y2 = y1; y1 = out; return out;
    }
};

class StateVariableFilter {
private: double lowState, bandState;
public:
    ...
    void process(double input, double cutoffHz, double resonance, double& lpOut, double& hpOut) {
        double f = 2.0 * std::sin(PI * cutoffHz / SAMPLE_RATE); f = std::clamp(f, 0.0, 1.0); double q = 1.0 / resonance;
        hpOut = input - lowState - q * bandState; bandState += f * hpOut; lpOut = lowState + f * bandState; lowState = lpOut;
    }
};
...
public:
    ...
    void initialize(double durationSec) {
        clear();
        if (durationSec <= 0.015) { activeWetMix = 0.0; return; }
        activeWetMix = 0.15 + (std::min(durationSec, 3.5) * 0.12);
        int cDelays[4] = {1103, 1249, 1399, 1559}; int aDelays[2] = {211, 89};
        for(int i = 0; i < 4; ++i) {
            double dSec = (double)cDelays[i] / SAMPLE_RATE;
            combs.push_back(new CombFilter(cDelays[i], durationSec, dSec));
        }
        allpasses.push_back(new AllPassFilter(aDelays[0], 0.70));
        allpasses.push_back(new AllPassFilter(aDelays[1], 0.62));
    }
    double process(double input) {
    ...
    void clear() {
        for(auto* c : combs) delete c; combs.clear();
        for(auto* ap : allpasses) delete ap; allpasses.clear();
    }
    ~SchroederReverb() { clear(); }
};

class HaasDelay {
private: std::vector<double> buffer; int writeIdx; int delaySamples;
public:
    HaasDelay() : writeIdx(0), delaySamples(0) {}
    void set_delay(double delaySec) {
        delaySamples = static_cast<int>(delaySec * SAMPLE_RATE);
        if (delaySamples < 1) delaySamples = 1;
        buffer.assign(delaySamples, 0.0); writeIdx = 0;
    }
    double process(double input) {
        if (buffer.empty()) return input;
        double output = buffer[writeIdx]; buffer[writeIdx] = input;
        if (++writeIdx >= delaySamples) writeIdx = 0; return output;
    }
};
...
double get_glottal_pulse(double phase) {
    ...
    else if (np < 0.30) return 1.0 - std::pow((np - 0.22) / 0.08, 2); return 0.0; 
}
...
void write_wav_header(std::ofstream& file, int data_size) {
    ...
    short af = 1; short nc = 2; int sr = SAMPLE_RATE; int br = SAMPLE_RATE * 2 * 2; short ba = 4; short bps = 16;
...
int main(int argc, char* argv[]) {
    // STREAMLINED USAGE MENU
    if (argc < 2) {
        std::cout << "Usage: ./reverb_tts [--filter-sine | --filter-singing | --filter-glitch] [--ringmod 0-100] [--reverb 0-100] [--pan 0-100] [--haas 0-100] \"<phrase>\"\n"; return 1;
    }
    
    EngineMode selected_mode = MODE_FILTER_SINE; 
    std::string raw_string = ""; 
    int reverbValue = 0; int ringmodValue = -1; int panValue = -1; int haasValue = -1;
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--ringmod" && i + 1 < argc) { ringmodValue = std::atoi(argv[++i]); }
        else if (arg == "--reverb" && i + 1 < argc) { reverbValue = std::atoi(argv[++i]); }
        else if (arg == "--pan" && i + 1 < argc) { panValue = std::atoi(argv[++i]); }
        else if (arg == "--haas" && i + 1 < argc) { haasValue = std::atoi(argv[++i]); }
        else {
            if (raw_string.empty() && arg.rfind("--", 0) != 0) raw_string = arg;
        }
    }
    if (raw_string.empty()) { std::cout << "[-] Missing token string core text input stream.\n"; return 1; }
    
    reverbValue = std::clamp(reverbValue, 0, 100);
    double reverbDuration = 0.01 + ((double)reverbValue / 100.0) * (6.0 - 0.01);

    double rmCarrierHz = 0.0;
    bool enable_ring_mod = (ringmodValue >= 0);
    if (enable_ring_mod) {
        ringmodValue = std::clamp(ringmodValue, 0, 100);
        double norm = (double)ringmodValue / 100.0;
        rmCarrierHz = 1500.0 - (norm * (1500.0 - 30.0));
    }

    double panLfoHz = 0.0;
    bool enable_panning = (panValue >= 0);
    if (enable_panning) {
        panValue = std::clamp(panValue, 0, 100);
        panLfoHz = 0.1 + ((double)panValue / 100.0) * (15.0 - 0.1);
    }

    double haasDelaySec = 0.0;
    bool enable_haas = (haasValue >= 0);
    HaasDelay spatialThickener;
    if (enable_haas) {
        haasValue = std::clamp(haasValue, 0, 100);
        // Clean safe window baseline (maxes out at 35 ms)
        haasDelaySec = 0.008 + ((double)haasValue / 100.0) * (0.035 - 0.008); 
        spatialThickener.set_delay(haasDelaySec);
    }
    ...
    while (ss >> word) {
        std::string clean = clean_str(word); if (clean.empty()) continue;
        bool target_to_sine = (selected_mode == MODE_FILTER_SINE); bool is_singing = (selected_mode == MODE_FILTER_SINGING || selected_mode == MODE_FILTER_GLITCH);
        ...
        for (const auto& ph : targeted_phonemes) {
            ...
            if (PHONEMES.contains(ph)) total_stream_duration += PHONEMES[ph].duration;
        }
        master_stream.push_back({" ", target_to_sine, is_singing}); total_stream_duration += 0.06;
    }

    std::string filename = "stereo_output2.wav"; std::vector<short> pcm;
    double phase_f0 = 0.0, phase_f1 = 0.0, phase_f2 = 0.0, phase_f3 = 0.0, phase_fric_a = 0.0, phase_fric_b = 0.0;
    double hybrid_p0 = 0.0, noise_lpf_state = 0.0, phase_ring_mod = 0.0; unsigned int noise_seed = 54321;
    
    Resonator R1, R2, R3; StateVariableFilter morphFilter; SchroederReverb acousticReverb;
    acousticReverb.initialize(reverbDuration);
    ...
    for (const auto& scheduled : master_stream) {
        ...
        int len = static_cast<int>(ph.duration * SAMPLE_RATE); int burst = static_cast<int>(ph.burst_duration * SAMPLE_RATE);
        ...
        for (int s = 0; s < len; ++s) {
            ...
            double current_f0 = F0_STATIC; double currentTime = (double)global_sample_index / SAMPLE_RATE;

            if (scheduled.is_singing_word) {
                current_f0 = 165.0 + 45.0 * std::sin(2.0 * PI * 0.85 * currentTime) + 8.0 * std::sin(2.0 * PI * 6.2 * currentTime);
                if (selected_mode == MODE_FILTER_GLITCH && std::sin(2.0 * PI * 0.4 * currentTime) > 0.3) {
            ...
            if (scheduled.use_sine_engine) {
                phase_f0 += (2.0 * PI * current_f0) / SAMPLE_RATE; phase_f1 += (2.0 * PI * f1) / SAMPLE_RATE;
                phase_f2 += (2.0 * PI * f2) / SAMPLE_RATE; phase_f3 += (2.0 * PI * f3) / SAMPLE_RATE;
                if (phase_f0 > 2.0 * PI) phase_f0 -= 2.0 * PI; if (phase_f1 > 2.0 * PI) phase_f1 -= 2.0 * PI;
                if (phase_f2 > 2.0 * PI) phase_f2 -= 2.0 * PI; if (phase_f3 > 2.0 * PI) phase_f3 -= 2.0 * PI;

                if (ph.is_plosive && s < burst) { raw_sample = std::sin(phase_f1 * 2.8) * (1.0 - ((double)s / burst)) * 0.55; }
                else if (ph.is_fricative) {
                    phase_fric_a += (2.0 * PI * f2) / SAMPLE_RATE; phase_fric_b += (2.0 * PI * (f3 * 1.25)) / SAMPLE_RATE;
                    if (phase_fric_a > 2.0 * PI) phase_fric_a -= 2.0 * PI; if (phase_fric_b > 2.0 * PI) phase_fric_b -= 2.0 * PI;
                    raw_sample = std::sin(phase_fric_a) * std::sin(phase_fric_b * 1.5) * 0.22;
                } else {
                    raw_sample = (std::sin(phase_f0) * 0.4) + (std::sin(phase_f1) * 0.35) + (std::sin(phase_f2) * 0.20) + (std::sin(phase_f3) * 0.10);
                }
                raw_sample *= 0.85;
            } else {
                double b1 = ph.start.b1 + r * (ph.end.b1 - ph.start.b1); double b2 = ph.start.b2 + r * (ph.end.b2 - ph.start.b2);
                double b3 = ph.start.b3 + r * (ph.end.b3 - ph.start.b3); R1.set_parameters(f1, b1); R2.set_parameters(f2, b2); R3.set_parameters(f3, b3);
                hybrid_p0 += (2.0 * PI * current_f0) / SAMPLE_RATE; if (hybrid_p0 > 2.0 * PI) hybrid_p0 -= 2.0 * PI;
                noise_seed = noise_seed * 1103515245 + 12345; double n = ((double)(noise_seed % 2000) / 1000.0) - 1.0;
                noise_lpf_state = 0.45 * n + 0.55 * noise_lpf_state;
                double src = 0.0;
                if (ph.is_plosive && ph.is_fricative) src = (s < burst) ? noise_lpf_state * (1.2 - ((double)s / burst)) * 1.8 : noise_lpf_state * 0.65;
                else if (s < burst) src = noise_lpf_state * (1.0 - ((double)s / burst)) * 1.5;
                else if (ph.is_fricative) src = noise_lpf_state * 0.70;
                else { double p = get_glottal_pulse(hybrid_p0) * 1.6; src = p - 0.15 * std::pow(p, 3); }
            ...
            if (enable_ring_mod) {
                phase_ring_mod += (2.0 * PI * rmCarrierHz) / SAMPLE_RATE; if (phase_ring_mod > 2.0 * PI) phase_ring_mod -= 2.0 * PI;
            ...
            double leftGain = 0.7071; double rightGain = 0.7071;
            if (enable_panning) {
                double panControl = 0.5 + 0.5 * std::sin(2.0 * PI * panLfoHz * currentTime);
                leftGain = std::cos(panControl * (PI / 2.0)); rightGain = std::sin(panControl * (PI / 2.0));
            }

            double finalLeft = processed_sample * leftGain; double finalRight = processed_sample * rightGain;
            if (enable_haas) finalRight = spatialThickener.process(finalRight);

            pcm.push_back(static_cast<short>(std::clamp(finalLeft, -1.0, 1.0) * 32767));
            pcm.push_back(static_cast<short>(std::clamp(finalRight, -1.0, 1.0) * 32767));
            global_sample_index++;
        }
    }

    int tailSamples = static_cast<int>(reverbDuration * SAMPLE_RATE * 1.5);
    for (int t = 0; t < tailSamples; ++t) {
        double tail = acousticReverb.process(0.0); tail = std::clamp(tail, -1.0, 1.0);
        ...
        double finalLeft = tail * leftGain; double finalRight = tail * rightGain;
        if (enable_haas) finalRight = spatialThickener.process(finalRight);
    ...
    std::cout << "[+] Rendering complete. Engine Mode: ";
    if (selected_mode == MODE_FILTER_SINE) std::cout << "Sine-Wave Formant\n";
    else if (selected_mode == MODE_FILTER_SINGING) std::cout << "Cascade Filter Singing\n";
    else std::cout << "Cascade Filter Tangent Glitch\n";

    std::cout << "[+] Reverb context size: " << reverbValue << "/100 (" << reverbDuration << "s).\n";
    if (enable_ring_mod) std::cout << "[+] Ringmod factor: " << ringmodValue << "/100 (" << rmCarrierHz << " Hz)\n";
    if (enable_panning) std::cout << "[+] Constant-Power Panning Rate: " << panValue << "/100 (" << panLfoHz << " Hz Auto-Sweep)\n";
    if (enable_haas) std::cout << "[+] Haas Acoustic Delay window: " << haasValue << "/100 (" << haasDelaySec * 1000.0 << " ms)\n";
